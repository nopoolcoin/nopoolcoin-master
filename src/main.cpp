...
uint256 hashGenesisBlock("0x01b2ba31dc1f4a62649c7bf753ace744f600815a443bb544de07878d83877bfd");
static const unsigned int timeGenesisBlock = 1437731190;// changedxx

static CBigNum bnProofOfWorkLimit(~uint256(0)>>6);
...
const string strMessageMagic = "nopoolcoin Signed Message:\n";
...
int64 static GetBlockValue(int nHeight, int64 nFees,unsigned int nBits)
{
		...
		//CBigNum bnNew,bnNew2,bnNew3;
		//bnNew.SetCompact( nBits);
		//bnNew2.SetCompact(bnProofOfWorkLimit.GetCompact());
		
		//printf("nBits1:%d\n", nBits);
		//bnNew3 = bnNew2/bnNew;
		//unsigned long a = bnNew3.getulong();
    
		//nshare = nGenesisBlockCoin* 99/a;	// Initial share 
    	//if ((nshare < nBlockRewardMinimumCoin)||(nHeight>=nUpdatePoint))
    	//{
		//	nshare = nBlockRewardMinimumCoin;
		//}

    	//return nshare + nFees*2;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    if (((pindexLast->nHeight+1) % nInterval != 0)&&((pindexLast->nHeight+1)!=nUpdatePoint))
    {
    ...
    CBigNum bnNew;
    int64 LimUp = nTargetTimespan /2; // 200% up
    int64 LimDown = nTargetTimespan * 4; //110% down per 4 Timespan

	
    if((pindexLast->nHeight+1)<nUpdatePoint)
    {
    		if (nActualTimespan < LimUp)
		{
		
			printf("GetNextWorkRequired nActualTimespan < LimUp\n");
        		nActualTimespan = LimUp;
			bnNew.SetCompact(pindexLast->nBits);
			bnNew *= nActualTimespan;
			bnNew /= nTargetTimespan;
			
		}else if (nActualTimespan > LimDown)
		{
		
			printf("GetNextWorkRequired nActualTimespan > LimDown\n");
        		nActualTimespan = LimDown / 4 * 1.1;
			bnNew.SetCompact(pindexLast->nBits);
			bnNew *= nActualTimespan;
			bnNew /= nTargetTimespan;
		}
		...
		{
			printf("GetNextWorkRequired unchange\n");
			bnNew.SetCompact(pindexLast->nBits);
		}
    	}
	else if((pindexLast->nHeight+1)==nUpdatePoint)//
	{
		
		printf("GetNextWorkRequired upadate\n");
		bnNew.SetCompact(pindexLast->nBits);
		bnNew *= 64;
	}
	else
	{
    	if (nActualTimespan < LimUp)
		{
			...
			printf("GetNextWorkRequired nActualTimespan < LimUp\n");
			bnNew.SetCompact(pindexLast->nBits);
			nTempTimespan = nActualTimespan * 1.5;
			bnNew *= nTempTimespan;
			bnNew /= nTargetTimespan;
		}else if (nActualTimespan > LimDown)
		{
...
uint256 CBlockHeader::GetHash() const
{
	...
	for (i = 0; i < 88; i++)
	{
        	sprintf(psz + i*2, "%02x", ((unsigned char*)&nVersion)[i]);
	}
...
//		printf("GetHash - MidHash %s\n", midHash.ToString().c_str());
	...
	//printf("GetHash - Birthday B %u hash \n", nBirthdayB);
    		...
    		uint256 temp = Hash(BEGIN(nVersion), END(nBirthdayB));
		uint256 result;
    		result =Hashkeccak2((unsigned char*)&temp, sizeof(temp));
...
//    	fprintf( stderr, "init hash %s\n", r.ToString().c_str() );
		if(!bts::momentum_verify( midHash, nBirthdayA, nBirthdayB)){
    		...
    		return result; //Hash(BEGIN(nVersion), END(nBirthdayB));
}
...
uint256 CBlockHeader::CalculateBestBirthdayHash(int& collisions) {
		...
		if(!(bts::momentum_search( midHash ,results)))
		{
		...
		for (unsigned i=0; i < results.size(); i++) {
			...
			uint256 temp = Hash(BEGIN(nVersion), END(nBirthdayB));
    			uint256 fullHash =Hashkeccak2((unsigned char*)&temp, sizeof(temp));
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=000000000019d6, ver=1, hashPrevBlock=00000000000000, hashMerkleRoot=4a5e1e, nTime=1437440400, nBits=1d00ffff, nNonce=2083236893, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "tests will be end!!!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04a61f478e6912f6220e8bd8ff031c66c9afaa69a10de9e42cf8d622afc5828142eb24a5f5fdc76dd7aa114b4812b2f36b82e338945a4bf7eec7713cc239df7719") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1437731190;//timeGenesisBlock;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 119201913;
        block.nBirthdayA = 3615989;
        block.nBirthdayB = 4123332;

        if (fTestNet)
        {
            block.nTime    = 1437731190;
        ...
        block.print();
        static const uint256 hashGenesisMerkleRoot = uint256("0x49cf509cf61c907b1d8865c0e6035451d46ffd512262a5e16ffb3d90f2376bce");
        assert(block.hashMerkleRoot == hashGenesisMerkleRoot);
        hashGenesisBlock = uint256("0x01b2ba31dc1f4a62649c7bf753ace744f600815a443bb544de07878d83877bfd");
...
unsigned char pchMessageStart[4] = { 0xd9, 0xf8, 0xa7, 0xf6 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("nopoolcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("nopoolcoinMiner : ProcessBlock, block not accepted");
    }
...
*/
    ...
    if (minerThreads == NULL){
    	...
    	//printf("BitcoinMiner nThreads:%d\n",nThreads);
	}
}
